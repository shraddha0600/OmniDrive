...
import { AiSearchChatbot } from '@/components/drive/AiSearchChatbot'
import { DocumentChatDrawer } from '@/components/drive/DocumentChatDrawer'
...
function Sidebar({ onNavigate, user, storage, breakdown, onLogout }: { onNavigate?: () => void; user: AuthUser | null; storage: StorageSummary | null; breakdown: StorageBreakdown; onLogout: () => void }) {
  ...
  const photoBytes = Number(breakdown.photo || 0)
  const videoBytes = Number(breakdown.video || 0)
  const docBytes = Number(breakdown.document || 0)

  const photoPct = total > 0 ? (photoBytes / total) * 100 : 0
  const videoPct = total > 0 ? (videoBytes / total) * 100 : 0
  const docPct = total > 0 ? (docBytes / total) * 100 : 0

  const items = [
    ['Photo', formatBytes(breakdown.photo), 'bg-lime-500'],
    ['Video', formatBytes(breakdown.video), 'bg-amber-400'],
    ['Document', formatBytes(breakdown.document), 'bg-cyan-400'],
    ['Free Storage', formatBytes(storage?.availableBytes), 'bg-indigo-500'],
  ]
  ...
  return (
    <aside className="flex h-full w-64 flex-col border-slate-200/60 dark:border-white/10 bg-slate-50/40 dark:bg-slate-950/40 backdrop-blur-2xl p-4 lg:border-r">
      <div className="flex items-center gap-2.5 pb-3 pt-1">
        <BrandLogo className="h-8 w-8" />
        <span className="text-xl font-extrabold tracking-tight bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">OmniDrive AI</span>
      </div>

      <div className="flex items-center gap-2.5 border-y border-slate-200/60 dark:border-white/10 py-3 my-3">
      ...
      <nav className="grid gap-1">
        {menu.map((item) => item.disabled ? (
          ...
          <NavLink key={item.label} to={item.href} onClick={onNavigate} className={({ isActive }) => cn('inline-flex h-10 items-center gap-2.5 rounded-xl px-3.5 text-[13px] font-bold transition-all border border-transparent', isActive ? 'bg-blue-600/10 text-blue-600 border-blue-600/20 shadow-sm dark:bg-blue-500/20 dark:text-blue-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white')}>
            <item.icon className="h-4 w-4" />
            {item.label}
          </NavLink>
        ))}
      ...
      {/* Glassmorphic Storage Sidebar Widget */}
      <div className="mt-auto border-t border-slate-200/60 dark:border-white/10 pt-4">
        <div className="rounded-2xl border border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-slate-900/60 p-3.5 shadow-sm backdrop-blur-xl">
          <div className="flex items-center justify-between text-xs font-bold text-slate-900 dark:text-slate-100 mb-2">
            <span>Storage Used</span>
            <span className="text-blue-600 dark:text-blue-400">{progress.toFixed(1)}%</span>
          </div>
          ...
          <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-slate-200/80 dark:bg-slate-800">
            <div className="bg-lime-500 h-full transition-all" style={{ width: `${photoPct}%` }} title={`Photo: ${formatBytes(breakdown.photo)}`} />
            <div className="bg-amber-400 h-full transition-all" style={{ width: `${videoPct}%` }} title={`Video: ${formatBytes(breakdown.video)}`} />
            <div className="bg-cyan-400 h-full transition-all" style={{ width: `${docPct}%` }} title={`Document: ${formatBytes(breakdown.document)}`} />
          </div>

          <div className="mt-3 grid gap-1">
            {items.map(([label, value, color]) => (
              <div key={label} className="flex items-center justify-between text-[11px] font-medium text-slate-500 dark:text-slate-400">
          ...
          <button
            ...
            className="mt-3 w-full rounded-xl bg-blue-50 dark:bg-blue-950/50 py-1.5 text-center text-xs font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/60 transition-colors"
          >
            Manage Storage
          </button>
        </div>
...
export function DriveLayout() {
  ...
  const [docChatFile, setDocChatFile] = useState<FileItem | null>(null)
  const { uploadProgress, setUploadProgress, retryFailedUpload } = useUpload()

  useEffect(() => {
    const onOpenChat = (e: Event) => {
      const customEvent = e as CustomEvent<FileItem>
      if (customEvent.detail) setDocChatFile(customEvent.detail)
    }
    window.addEventListener('omnidrive:open-doc-chat', onOpenChat)
    return () => window.removeEventListener('omnidrive:open-doc-chat', onOpenChat)
  }, [])
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      <CommandPalette
      ...
      <div className="flex min-h-screen w-full flex-col lg:h-screen lg:overflow-hidden lg:flex-row">
        ...
        <div className={cn('fixed inset-0 z-40 bg-slate-950/40 backdrop-blur-sm transition-opacity lg:hidden', sidebarOpen ? 'opacity-100' : 'pointer-events-none opacity-0')} onClick={() => setSidebarOpen(false)} />
        <div className={cn('fixed inset-y-0 left-0 z-50 transform bg-white dark:bg-slate-950 shadow-2xl transition-transform duration-300 ease-out lg:hidden', sidebarOpen ? 'translate-x-0' : '-translate-x-full')}>
            ...
            <div className="flex items-center justify-between gap-3 lg:hidden">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="truncate text-xl font-extrabold tracking-tight">OmniDrive AI</span>
                </div>
              </div>
            ...
            <div className="relative w-full min-w-0 flex-1 lg:max-w-sm xl:max-w-xl">
              <form onSubmit={searchFiles} className="relative w-full">
                ...
                <Input
                  ...
                  placeholder="Search files or type ⌘K..."
                  onClick={() => setCommandPaletteOpen(true)}
                  className="pl-10 pr-20 bg-slate-50/80 dark:bg-slate-900/60 border-slate-200/80 dark:border-white/10 rounded-2xl"
                />
                <button
                  type="button"
                  onClick={() => setCommandPaletteOpen(true)}
                  className="absolute right-12 top-1/2 -translate-y-1/2 hidden sm:inline-flex items-center gap-1 rounded border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 text-[10px] font-bold text-slate-400"
                >
                  ⌘K
                </button>
                <button type="button" onClick={() => setFiltersOpen(!filtersOpen)} className={cn("absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors", filtersOpen && "text-blue-600 hover:text-blue-700")} aria-label="Search filters"><SlidersHorizontal className="h-4 w-4" /></button>
              </form>

              {filtersOpen && (
                <div className="absolute left-0 right-0 top-12 z-50 rounded-2xl border border-slate-200 dark:border-white/10 bg-white/95 dark:bg-slate-900/95 p-5 shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/10 pb-3">
                    <span className="text-sm font-extrabold text-slate-950 dark:text-slate-100">Advanced Search Filters</span>
                  ...
                  <div className="mt-4 grid gap-4 sm:grid-cols-2">
                    {/* File Kind */}
                    <div>
                      <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">File Type</label>
                      <select value={filterKind} onChange={(e) => setFilterKind(e.target.value)} className="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
                    ...
                    {/* Connected Account */}
                    <div>
                      <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Connected Account</label>
                      <select value={filterAccountId} onChange={(e) => setFilterAccountId(e.target.value)} className="mt-1 block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
                    ...
                    {/* Size range */}
                    <div>
                      <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Size Range (MB)</label>
                      <div className="mt-1 flex items-center gap-2">
                        <input type="number" placeholder="Min" value={filterMinSize} onChange={(e) => setFilterMinSize(e.target.value)} className="block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
                        <span className="text-slate-400 text-xs font-semibold">to</span>
                        <input type="number" placeholder="Max" value={filterMaxSize} onChange={(e) => setFilterMaxSize(e.target.value)} className="block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
                      </div>
                    </div>

                    {/* Date range */}
                    <div>
                      <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Date Range</label>
                      <div className="mt-1 flex items-center gap-2">
                        <input type="date" value={filterStartDate} onChange={(e) => setFilterStartDate(e.target.value)} className="block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
                        <span className="text-slate-400 text-xs font-semibold">to</span>
                        <input type="date" value={filterEndDate} onChange={(e) => setFilterEndDate(e.target.value)} className="block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
                      </div>
                    </div>
                  </div>

                  <div className="mt-5 flex justify-end gap-2 border-t border-slate-100 dark:border-white/10 pt-4">
      ...
      {/* Floating Upload Progress Panel with Glassmorphism */}
      {uploadProgress.open ? (
        <div className="fixed inset-x-3 bottom-3 z-[70] max-h-[70dvh] overflow-hidden rounded-2xl border border-slate-200/80 dark:border-white/10 bg-white/90 dark:bg-slate-900/90 shadow-2xl backdrop-blur-2xl transition-all duration-300 sm:inset-x-auto sm:bottom-5 sm:right-5 sm:w-[min(420px,calc(100vw-2.5rem))]">
          <div className="flex items-center justify-between border-b border-slate-200/60 dark:border-white/10 px-4 py-3 bg-slate-50/50 dark:bg-slate-950/40">
            <div className="flex items-center gap-2 font-extrabold text-sm text-slate-950 dark:text-slate-100">
            ...
            <div className="flex items-center gap-1">
              ...
              >
                <ChevronDown className={cn("h-4 w-4 transition-transform duration-200", uploadProgressCollapsed && "rotate-180")} />
              </Button>
              <Button
                variant="ghost"
                size="icon"
          ...
          {!uploadProgressCollapsed && (
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between gap-3 text-sm">
                <p className="truncate font-bold text-slate-900 dark:text-slate-100">{uploadProgress.fileName}</p>
                <span className="font-extrabold text-blue-600 dark:text-blue-400">{uploadProgress.percent}%</span>
              </div>
                ...
                <div
                  className={
                    uploadProgress.status === 'error' || uploadProgress.status === 'partial'
                      ...
                      : 'h-full rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 transition-all duration-300'
                  }
              ...
              {uploadProgress.files.length > 0 ? (
                <div className="mt-3 grid max-h-60 gap-2.5 overflow-y-auto pr-1">
                  {uploadProgress.files.map((file, index) => (
                    <div
                      key={`${file.name}-${file.size}-${index}`}
                      className="grid gap-1.5 rounded-xl border border-slate-200/60 dark:border-white/5 bg-slate-50/80 dark:bg-slate-950/60 p-3"
                    >
                      <div className="flex min-w-0 items-center justify-between gap-3 text-xs">
                        <p className="min-w-0 flex-1 truncate font-bold text-slate-900 dark:text-slate-100" title={file.name}>
                          {file.name}
                        </p>
                        <span className="shrink-0 font-semibold text-slate-500">{file.percent}%</span>
                      </div>
                      <div className="flex items-center justify-between gap-3 text-[11px] text-slate-500 dark:text-slate-400">
                        ...
                        <div className="flex items-center gap-2">
                          {file.status === 'error' && (
                            <Button
                              variant="default"
                              className="h-6 px-2 text-[10px] font-extrabold text-white bg-blue-600 hover:bg-blue-700 shadow-none border-none rounded-lg"
      ...
      {/* Floating AI Search Chatbot */}
      <AiSearchChatbot />

      {/* Document Q&A Side Panel Drawer */}
      <DocumentChatDrawer
        isOpen={!!docChatFile}
        file={docChatFile}
        onClose={() => setDocChatFile(null)}
      />
    </main>
  )
}
