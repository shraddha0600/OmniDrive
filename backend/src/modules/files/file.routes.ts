...
import { parseNlpQuery, NLP_SYSTEM_PROMPT } from './nlp-search-parser.js'
...
fileRouter.post('/nlp-search', async (req: AuthRequest, res, next) => {
  try {
    const { query } = z.object({ query: z.string().trim().min(1).max(500) }).parse(req.body)
    const intent = await parseNlpQuery(query)

    const typeFilters: Record<string, string[]> = {
      image: ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml'],
      video: ['video/mp4', 'video/mpeg', 'video/ogg', 'video/quicktime', 'video/webm'],
      pdf: ['application/pdf'],
      doc: ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'text/plain', 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', 'text/markdown'],
    ...
    const where: any = {
      userId: req.user!.id,
      status: 'active',
      ...(intent.categoryTag !== 'All' ? { category: intent.categoryTag } : {}),
      ...(intent.kind ? { mimeType: { in: typeFilters[intent.kind] || [] } } : {}),
      ...(intent.minSizeBytes !== undefined || intent.maxSizeBytes !== undefined ? {
        sizeBytes: {
          ...(intent.minSizeBytes !== undefined ? { gte: BigInt(intent.minSizeBytes) } : {}),
          ...(intent.maxSizeBytes !== undefined ? { lte: BigInt(intent.maxSizeBytes) } : {})
        }
      } : {}),
      ...(intent.startDate || intent.endDate ? {
        createdAt: {
          ...(intent.startDate ? { gte: new Date(intent.startDate) } : {}),
          ...(intent.endDate ? { lte: new Date(intent.endDate) } : {})
        }
      } : {})
    }
    ...
    // Compute / backfill categories for retrieved files if missing or Other
    const processedFiles = await Promise.all(files.map(async (file) => {
      let cat = file.category
      const computed = classifyFile(file.name, file.mimeType)
      if (!cat || cat === 'Other') {
        cat = computed
        if (computed !== 'Other') {
          await prisma.file.update({ where: { id: file.id }, data: { category: computed } }).catch(() => undefined)
        }
      }
      return { ...file, category: cat || 'Other' }
    }))

    // Smart Relevance Scoring
    const scoredFiles = processedFiles.map((file) => {
    ...
    return res.json({
      intent,
      systemPromptUsed: NLP_SYSTEM_PROMPT,
      files: finalFiles
    })
  } catch (error) {
    return next(error)
...
fileRouter.post('/:id/chat', async (req: AuthRequest, res, next) => {
  try {
    const fileId = String(req.params.id)
    const { question } = z.object({ question: z.string().trim().min(1).max(1000) }).parse(req.body)
    ...
    const category = file.category || classifyFile(file.name, file.mimeType)
    const folderInfo = file.folder ? `Folder: ${file.folder.name}` : 'Folder: Root'

    const docContext = `Document Name: ${file.name}\nCategory: ${category}\nType: ${file.mimeType}\nSize: ${file.sizeBytes} bytes\n${folderInfo}`

    const systemPrompt = `You are OmniDrive AI Document Assistant. You are answering questions about the following document in the user's storage:\n\n${docContext}\n\nProvide an accurate, clear, helpful answer to the user's question about this file. If asked for a summary, give key bullet points.`

    const ollamaUrl = env.OLLAMA_URL
    const modelName = env.OLLAMA_MODEL

    let answer = ''
    let engineUsed = 'ollama:minimax-m2.5:cloud'
    ...
    if (!answer) {
      answer = `Based on document "${file.name}" (Category: ${category}): This is a ${file.mimeType} file stored in your drive. If you'd like a deep content summary, make sure your local Ollama instance is active.`
    }
...
fileRouter.get('/', async (req: AuthRequest, res, next) => {
  try {
    ...
    // Compute / backfill categories for all retrieved files
    const processedFiles = await Promise.all(files.map(async (file) => {
      let cat = file.category
      const computed = classifyFile(file.name, file.mimeType)
      if (!cat || cat === 'Other') {
      ...
      return { ...file, category: cat || 'Other', sizeBytes: file.sizeBytes.toString() }
    }))

    // Filter by category if specified
    const targetCategory = query.category && query.category !== 'All' ? query.category : null
    const finalFiles = targetCategory
      ? processedFiles.filter((f) => f.category.toLowerCase() === targetCategory.toLowerCase())
      : processedFiles
