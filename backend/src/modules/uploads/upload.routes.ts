...
import { classifyFile } from '../files/file-classifier.js'
      ...
      try {
        ...
        if (account.provider === 's3') {
          ...
          const provisionalFile = await prisma.file.create({
            data: { userId: req.user!.id, connectedAccountId: account.id, folderId, provider: 's3', providerFileId: 'pending', name: fileName, mimeType: meta.mimeType, sizeBytes: meta.sizeBytes, status: 'uploading', category: classifyFile(fileName, meta.mimeType) },
          })
        ...
        const file = account.provider === 's3' ? null : await prisma.file.create({ data: { userId: req.user!.id, connectedAccountId: account.id, folderId, provider: 'google_drive', providerFileId, name: uploadedName, mimeType: uploadedMimeType, sizeBytes: meta.sizeBytes, category: classifyFile(uploadedName, uploadedMimeType) } })
        if (file) {
...
uploadRouter.put('/resumable/chunk/:id', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    if (putRes.ok) {
      ...
      if (!existingFile) {
        const finalName = fileMeta.name || session.fileName
        const finalMime = fileMeta.mimeType || session.mimeType
        existingFile = await prisma.file.create({
          data: {
            ...
            category: classifyFile(finalName, finalMime)
          }
        })
      }
