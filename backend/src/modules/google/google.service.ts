  ...
  for (const driveFile of driveFiles) {
    ...
    if (!existing) {
      await prisma.file.create({
        data: { userId, connectedAccountId: account.id, provider: 'google_drive', providerFileId: driveFile.id, name: driveFile.name, mimeType: driveFile.mimeType, sizeBytes: driveFile.sizeBytes, status: 'active', folderId: dbFolderId, category: classifyFile(driveFile.name, driveFile.mimeType) },
      })
      created += 1
      continue
    }
    ...
    if (needsUpdate) {
      await prisma.file.update({
        ...
        data: { name: driveFile.name, mimeType: driveFile.mimeType, sizeBytes: driveFile.sizeBytes, status: 'active', deletedAt: null, folderId: dbFolderId, category: classifyFile(driveFile.name, driveFile.mimeType) },
      })
      updated += 1
    }
  }
