...
OmniDrive AI is a smart Google Drive storage gateway with AI auto-tagging and natural language search. It lets users register/login with email/password or Google, automatically connect the first Drive account during Google sign-in, connect additional Google Drive accounts, track combined quota, upload files through the backend into a dedicated Google Drive folder, organize files in virtual folders, preview/download/share files, sync MySQL/SQLite file records from Google Drive, invite other users to files/folders, and route uploads to a connected Drive account with enough free space.
